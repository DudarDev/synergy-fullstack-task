...
try:
    ...
    print("База даних недоступна. Пропускаємо створення таблиць для тестів.")
...
async def sync_data(db: Session = Depends(get_db)):
    ...
    async with httpx.AsyncClient() as client:
        ...
        if users_resp.status_code != 200 or posts_resp.status_code != 200:
            raise HTTPException(
                status_code=400, detail="Помилка при отриманні даних з DummyJSON"
            )
        ...
        for u in users_data:
            existing_user = (
                db.query(models.User).filter(models.User.email == u["email"]).first()
            )
        ...
        for p in posts_data:
            existing_post = (
                db.query(models.Post).filter(models.Post.id == p["id"]).first()
            )
            if not existing_post:
                # Перевіряємо, чи існує автор цього поста в нашій базі
                user_exists = (
                    db.query(models.User).filter(models.User.id == p["userId"]).first()
                )
                if user_exists:
                    db_post = models.Post(
                        id=p["id"],
...
@app.get(
    "/posts", response_model=List[schemas.PostResponse], summary="Отримати всі пости"
)
